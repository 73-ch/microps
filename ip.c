...
struct ip_iface *ip_iface_alloc(const char *addr, const char *netmask) {
    ...
    if (ip_addr_pton(addr, &iface->unicast)) {
        errorf("ip addr cast error, %s\n", addr);
        memory_free(iface);
        return NULL;
    }

    if (ip_addr_pton(netmask, &iface->netmask)) {
        errorf("netmask cast error, %s\n", netmask);
        memory_free(iface);
        return NULL;
    }
...
static void ip_input(const uint8_t *data, size_t len, struct net_device *dev) {
    ...
    v = (uint8_t) ((hdr->vhl & 0xf0) >> 4);
    if (v != IP_VERSION_IPV4) {
        errorf("ip version error: not supported version %u", v);
        return;
    }
    ...
    if (len < total) {
        errorf("input data length(%u) less than total length(%u)", len, total);
        return;
    }

    if (cksum16((uint16_t *) data, len, 0) != 0) {
        errorf("input data cksum error");
        return;
    }
    ...
    iface = ifaces;
    if (!iface) {
        errorf("there is no valid iface.");
        return;
    }

    if (hdr->dst != iface->unicast && hdr->dst != 0xff && hdr->dst != iface->broadcast) {
        return;
    }
...
ip_output_core(struct ip_iface *iface, uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst,
    ...
    hdr->vhl = (IP_VERSION_IPV4 << 4 & 0xf0) | (hlen & 0x0f);
    hdr->tos = 0;
    total = hton16(len);
    hdr->total = total;
    hdr->id = hton16(id);
...
ssize_t ip_output(uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst) {
    ...
    if (src == IP_ADDR_ANY) {
        ...
        if ((dst & iface->netmask) != (iface->unicast & iface->netmask) && dst == 0xff) {
