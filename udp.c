...
struct udp_pcb {
    ...
    int wait_count; /* num of thread using pcb */
};
...
static void udp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface) {
    ...
    if (!queue_push(&pcb->queue, (uint8_t *) entry)) {
        mutex_unlock(&mutex);
...
ssize_t udp_output(struct ip_endpoint *src, struct ip_endpoint *dst, const uint8_t *data, size_t len) {
    ...
    if (ip_output(IP_PROTOCOL_UDP, (const uint8_t *) hdr, total, src->addr, dst->addr) == -1) {
...
int udp_close(int id) {
    ...
    pcb = udp_pcb_get(id);
    if (!pcb) {
        errorf("udp_pcb_get() failed: id=%d", id);
        return -1;
    }
...
int udp_bind(int id, struct ip_endpoint *local) {
    ...
    if (exist) {
        errorf("pcb(%s) found", ip_endpoint_ntop(local, ep1, sizeof(ep1)));
        mutex_unlock(&mutex);
        return -1;
    }
...
ssize_t udp_send_to(int id, uint8_t *data, size_t len, struct ip_endpoint *foreign) {
    struct udp_pcb *pcb;
    struct ip_endpoint local;
...
ssize_t udp_receive_from(int id, uint8_t *buf, size_t size, struct ip_endpoint *foreign) {
    struct udp_pcb *pcb;
    struct udp_queue_entry *entry;
    ssize_t len;
