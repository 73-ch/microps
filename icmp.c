...
void icmp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface) {
    ...
    if (len < sizeof(*hdr)) {
        errorf("packet length(%u) is too short", len);
        return;
    }
    ...
    if (cksum16((uint16_t *)data, len, 0) != 0) {
        errorf("cksum error %u != %u", cksum16((uint16_t *)data, len, 0), hdr->sum);
        return;
    }
    ...
    switch(hdr->type) {
        case ICMP_TYPE_ECHO:
            ...
            icmp_output(ICMP_TYPE_ECHOREPLY, hdr->code, hdr->values, (const uint8_t *) hdr + 1, len-sizeof(*hdr), dst, src);
            break;
        default:
            break;
    }
